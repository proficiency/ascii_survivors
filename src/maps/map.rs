...
impl Map {
    ...
    pub fn from_xp_data(xp_data: &[u8], name: &str) -> Result<Self> {
...
pub fn create_test_map_for_level(level: Level) -> Map {
    let mut map = match level {
        Level::Grassland => load_or_generate_grassland_map(),
        Level::Dungeon => generate_dungeon_map(),
        Level::Rest => generate_rest_area_map(),
        Level::Survival => generate_survival_map(),
    };
...
fn load_or_generate_grassland_map() -> Map {
    let name = "Grassland";
    let xp_path = PathBuf::from("assets/maps/grassland.xp");

    if let Ok(bytes) = std::fs::read(&xp_path) {
        if let Ok(map) = Map::from_xp_data(&bytes, name) {
            return map;
        }
    }
...
fn generate_grassland_map(name: &str) -> Map {
...
fn generate_dungeon_map() -> Map {
    let mut map = Map::new(BASE_MAP_WIDTH, BASE_MAP_HEIGHT, "Dungeon");
    for x in 25..55 {
...
fn generate_rest_area_map() -> Map {
    let mut map = Map::new(BASE_MAP_WIDTH, BASE_MAP_HEIGHT, "Rest Area");
    map.tiles[40][25] = Tile::new(TileType::Empty);
    map
}

fn generate_survival_map() -> Map {
    let mut map = Map::new(BASE_MAP_WIDTH, BASE_MAP_HEIGHT, "Survival Mode");
