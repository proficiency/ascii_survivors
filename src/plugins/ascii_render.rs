...
impl Plugin for AsciiRenderPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, setup_ascii_renderer)
            .add_systems(
                Startup,
                crate::systems::setup_lighting_overlay.after(setup_ascii_renderer),
            )
            .add_systems(Update, resize_ascii_grid)
            .add_systems(Update, update_cinematic_target.before(update_cinematic_camera))
            .add_systems(Update, update_cinematic_camera)
            .add_systems(
                Update,
                render_ascii_canvas.in_set(crate::plugins::schedule::GameSet::Rendering),
            );
    }
}
...
fn update_cinematic_camera(
    ...
    if let Ok(mut transform) = camera_query.single_mut() {
        let center = grid.world_center();
        transform.translation = Vec3::new(
            center.x + cinematic.current_offset.x,
            center.y + cinematic.current_offset.y,
            999.0,
        );
    }
...
fn update_cinematic_target(
    ...
    if let Ok(player) = player_query.single() {
        ...
        let target = player.world_position - center;
...
fn render_ascii_canvas(
    ...
    for (index, cell) in frame.cells.iter().enumerate() {
        ...
        if base_x + cell_size.x > canvas.pixel_size.x
            || base_y + cell_size.y > canvas.pixel_size.y
        {
            continue;
        }
