...
pub fn update_lighting_overlay(
    ...
    player_lights: Query<(&GridPosition, &LightEmitter), With<PlayerTag>>,
    campfire_lights: Query<(&Campfire, &LightEmitter)>,
    player_occluders: Query<&GridPosition, With<PlayerTag>>,
    ...
    if overlay.size != grid.grid_size {
        resize_lighting_overlay(&mut overlay, &mut images, &grid, &mut sprite_query);
    }
    ...
    for (pos, emitter) in player_lights.iter() {
        let pos = screen_position(pos.world, camera_offset.0, overlay_size, overlay_scale);
        apply_light_basic(
            pos,
            emitter,
            &mut overlay.buffer,
...
fn gather_occluders(
    ...
    if let Some(player) = player_query.iter().next() {
        occluders.push(LightOccluder {
            center: screen_position(player.world, camera_offset, overlay_size, overlay_scale),
